...
```{r}
plot(lm(Y ~ log(Income)*MPC + Drivers*Miles + Tax, data=df))
```
...
```{r}
df <- df[!(rownames(df) %in% c("TX", "CA", "NY", "FL")),]
```
...
Looks like the model now explains the data better, can't say why Texas, California, New York and Florida have such an impact on the model's performance.